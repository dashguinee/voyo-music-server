...
// ============================================
// VIBE-SPECIFIC SECTIONS (For HomeFeed)
// ============================================

type VibeName = 'afro_heat' | 'chill' | 'party' | 'workout' | 'late_night';

/**
 * Get tracks by specific vibe score (for dedicated sections)
 * Uses direct database query, not RPC
 */
async function getTracksByVibe(vibe: VibeName, limit: number = 20): Promise<Track[]> {
  if (!supabaseConfigured) return [];

  try {
    const { data, error } = await getSupabase()
      .from('video_intelligence')
      .select('youtube_id, title, artist, thumbnail_url, play_count, primary_genre, cultural_tags')
      .order(vibe, { ascending: false })
      .gt(vibe, 0.6) // Only tracks with strong vibe signal
      .limit(limit);

    if (error) {
      console.error(`[Discovery] ${vibe} tracks error:`, error);
      return [];
    }
    ...
    return (data || []).map((t: any) => ({
      ...
      coverUrl: t.thumbnail_url || `https://i.ytimg.com/vi/${t.youtube_id}/hqdefault.jpg`,
      duration: 0,
      tags: t.cultural_tags || [],
      oyeScore: Math.round((t.play_count || 0) / 10),
      createdAt: new Date().toISOString(),
    }));
  } catch (err) {
    console.error(`[Discovery] ${vibe} tracks exception:`, err);
    return [];
  }
}
 ...
 */
export async function getTrendingByPlays(limit: number = 20): Promise<Track[]> {
  if (!supabaseConfigured) return [];

  try {
    const { data, error } = await getSupabase()
      .from('video_intelligence')
      .select('youtube_id, title, artist, thumbnail_url, play_count, cultural_tags')
      .order('play_count', { ascending: false })
      .gt('play_count', 0)
      .limit(limit);

    if (error) {
      console.error('[Discovery] Trending tracks error:', error);
      return [];
    }

    console.log(`[Discovery] TRENDING: ${data?.length || 0} tracks by play count`);
    return (data || []).map((t: any) => ({
      id: t.youtube_id,
      trackId: t.youtube_id,
      title: t.title || 'Unknown',
      ...
      duration: 0,
      tags: t.cultural_tags || [],
      oyeScore: t.play_count || 0,
      createdAt: new Date().toISOString(),
    }));
  } catch (err) {
    console.error('[Discovery] Trending tracks exception:', err);
    return [];
  }
}

// Vibe-specific exports for HomeFeed sections
export const getAfroHeatTracks = (limit = 20) => getTracksByVibe('afro_heat', limit);
export const getChillTracks = (limit = 20) => getTracksByVibe('chill', limit);
export const getPartyTracks = (limit = 20) => getTracksByVibe('party', limit);
export const getWorkoutTracks = (limit = 20) => getTracksByVibe('workout', limit);
export const getLateNightTracks = (limit = 20) => getTracksByVibe('late_night', limit);
